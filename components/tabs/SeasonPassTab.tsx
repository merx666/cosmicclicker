...
export default function SeasonPassTab({ onUnlockPremium }: SeasonPassTabProps) {
    const { bpLevel,
        ...
        bpClaimedFree,
        bpClaimedPremium,
        claimBpReward } = useGameStore(useShallow(state => ({ bpLevel: state.bpLevel, bpXp: state.bpXp, bpPremium: state.bpPremium, premiumVIP: state.premiumVIP, bpClaimedFree: state.bpClaimedFree, bpClaimedPremium: state.bpClaimedPremium, claimBpReward: state.claimBpReward })))
