...
let WorkflowIndexService = class WorkflowIndexService {
    ...
    async updateIndexInternal(workflow, dependencyUpdates) {
        ...
        try {
            ...
            this.logger.error(`Failed to update workflow ${dependencyUpdates.publishedVersionId ? 'published' : 'draft'} dependency index for workflow ${workflow.id}: ${error.message}`);
            this.errorReporter.error(error);
            return;
        }
        this.logger.debug(`Workflow ${dependencyUpdates.publishedVersionId ? 'published' : 'draft'} dependency index ${updated ? 'updated' : 'skipped'} for workflow ${workflow.id}`);
    }
