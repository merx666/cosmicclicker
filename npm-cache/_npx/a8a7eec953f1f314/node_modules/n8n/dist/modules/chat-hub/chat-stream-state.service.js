...
const STREAM_STATE_TTL = 5 * 60;
const CLEANUP_DELAY_MS = STREAM_STATE_TTL * constants_1.Time.seconds.toMilliseconds;
let ChatStreamStateService = class ChatStreamStateService {
    constructor(logger, instanceSettings, executionsConfig, globalConfig, redisClientService) {
        this.logger = logger;
    ...
    async setRedisState(sessionId, state) {
        if (!this.redisClient)
            return;
        try {
            await this.redisClient.setex(this.getStateKey(sessionId), STREAM_STATE_TTL, JSON.stringify(state));
        }
        catch (error) {
    ...
    async setRedisChunks(sessionId, chunks) {
        if (!this.redisClient)
            return;
        try {
            await this.redisClient.setex(this.getChunksKey(sessionId), STREAM_STATE_TTL, JSON.stringify(chunks));
        }
        catch (error) {
