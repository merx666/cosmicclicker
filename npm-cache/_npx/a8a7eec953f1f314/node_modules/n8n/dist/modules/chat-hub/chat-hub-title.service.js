...
let ChatHubTitleService = class ChatHubTitleService {
    ...
    async runTitleWorkflowAndGetTitle(user, workflowData, executionData) {
        ...
        if (!execution.status || execution.status !== 'success') {
            const message = this.executionService.getErrorMessage(execution) ?? 'Failed to generate a response';
            throw new n8n_workflow_1.OperationalError(message);
        }
        const title = this.executionService.getAIOutput(execution, chat_hub_constants_1.NODE_NAMES.TITLE_GENERATOR_AGENT);
        return title ?? null;
    }
