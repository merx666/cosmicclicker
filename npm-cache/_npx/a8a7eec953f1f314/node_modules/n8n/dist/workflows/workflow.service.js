...
let WorkflowService = class WorkflowService {
    ...
    async activateWorkflow(user, workflowId, options, publicApi = false) {
        const workflow = await this.workflowFinderService.findWorkflowForUser(workflowId, user, ['workflow:publish'], { includeActiveVersion: true });
        if (!workflow) {
        ...
        if (options?.name !== undefined || options?.description !== undefined) {
            ...
            await this.workflowHistoryService.updateVersion(versionIdToActivate, workflowId, updateFields);
        }
        const updatedWorkflow = await this.workflowRepository.findOne({
            where: { id: workflowId },
            relations: {
    ...
    _validateNodes(workflowId, nodes) {
        const nodesToValidate = nodes.reduce((acc, node) => {
            acc[node.name] = node;
            return acc;
        }, {});
        const validation = this.workflowValidationService.validateForActivation(nodesToValidate, this.nodeTypes);
