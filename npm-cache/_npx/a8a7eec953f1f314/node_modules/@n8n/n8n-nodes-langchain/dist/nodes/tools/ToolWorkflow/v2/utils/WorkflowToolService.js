...
class WorkflowToolService {
    ...
    createStructuredTool(name, description, func) {
        const collectedArguments = (0, fromAIToolFactory_1.extractFromAIParameters)(this.baseContext.getNode().parameters);
        if (collectedArguments.length === 0) {
            return new tools_1.DynamicTool({ name, description, func });
        }
        const schema = (0, fromAIToolFactory_1.createZodSchemaFromArgs)(collectedArguments);
