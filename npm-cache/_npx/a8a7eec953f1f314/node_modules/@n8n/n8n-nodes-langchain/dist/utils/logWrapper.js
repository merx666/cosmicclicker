...
const embeddingInputValidation_1 = require("./embeddings/embeddingInputValidation");
    ...
    return new Proxy(originalInstance, {
        get: (target, prop) => {
            ...
            if (originalInstance instanceof embeddings_1.Embeddings ||
                originalInstance instanceof openai_1.OpenAIEmbeddings ||
                originalInstance instanceof openai_1.AzureOpenAIEmbeddings) {
                if (prop === 'embedDocuments' && 'embedDocuments' in target) {
                    return async (documents) => {
                        const validatedDocuments = (0, embeddingInputValidation_1.validateEmbedDocumentsInput)(documents, executeFunctions.getNode());
                ...
                if (prop === 'embedQuery' && 'embedQuery' in target) {
                    return async (query) => {
                        const validatedQuery = (0, embeddingInputValidation_1.validateEmbedQueryInput)(query, executeFunctions.getNode());
                        connectionType = n8n_workflow_1.NodeConnectionTypes.AiEmbedding;
                        const { index } = executeFunctions.addInputData(connectionType, [
            ...
            if (originalInstance instanceof document_compressors_1.BaseDocumentCompressor) {
                if (prop === 'compressDocuments' && 'compressDocuments' in target) {
                    return async (documents, query) => {
                        ...
                        (0, helpers_1.logAiEvent)(executeFunctions, 'ai-document-reranked', { query });
                        executeFunctions.addOutputData(connectionType, index, [[{ json: { response } }]]);
                        return response;
                    };
                }
            ...
            if ((0, helpers_1.isToolsInstance)(originalInstance)) {
                if (prop === '_call' && '_call' in target) {
                    return async (query) => {
                        ...
                        (0, helpers_1.logAiEvent)(executeFunctions, 'ai-tool-called', { ...inputData, response });
                        executeFunctions.addOutputData(connectionType, index, [[{ json: { response } }]]);
